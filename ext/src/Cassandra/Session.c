...
void php_cassandra_session_init(cassandra_session_base *session)
{
  ...
  session->persist             = 0;
  session->default_consistency = PHP_CASSANDRA_DEFAULT_CONSISTENCY;
  session->default_page_size   = 5000;
  PHP5TO7_ZVAL_UNDEF(session->default_timeout);
...
void php_cassandra_session_destroy(cassandra_session_base *session)
{
  ...
  PHP5TO7_ZVAL_MAYBE_DESTROY(session->default_timeout);
}
                                   ...
                                   zval *return_value)
{
  ...
  consistency = session->default_consistency;
  page_size = session->default_page_size;
  timeout = PHP5TO7_ZVAL_MAYBE_P(session->default_timeout);
  ...
  switch (stmt->type) {
    ...
    case CASSANDRA_PREPARED_STATEMENT:
      ...
      future = cass_session_execute((CassSession *) session->session->data, single);
      break;
    case CASSANDRA_BATCH_STATEMENT:
      ...
      future = cass_session_execute_batch((CassSession *) session->session->data, batch);
      break;
    default:
  ...
  do {
    ...
    if (single && cass_result_has_more_pages(result)) {
      ...
      rows->session   = php_cassandra_add_ref(session->session);
      return;
    }
                                         ...
                                         zval *return_value)
{
  ...
  switch (stmt->type) {
    ...
    case CASSANDRA_PREPARED_STATEMENT:
      ...
      future_rows->future    = cass_session_execute((CassSession *) session->session->data, single);
      future_rows->session   = php_cassandra_add_ref(session->session);
      break;
    case CASSANDRA_BATCH_STATEMENT:
      batch = create_batch(stmt, consistency, retry_policy, timestamp TSRMLS_CC);
      ...
      future_rows->future = cass_session_execute_batch((CassSession *) session->session->data, batch);
      break;
    default:
      INVALID_ARGUMENT(statement,
        "an instance of Cassandra\\SimpleStatement, " \
                                   ...
                                   zval *return_value)
{
  ...
  future = cass_session_prepare_n((CassSession *)session->session->data,
                                 ...
                                 zval *return_value)
{
  ...
  future = cass_session_close((CassSession *) session->session->data);
                                       ...
                                       zval *return_value)
{
  ...
  future->future = cass_session_close((CassSession *) session->session->data);
}
                                  ...
                                  zval *return_value)
{
  ...
  schema->schema = php_cassandra_new_ref(
                     (void *) cass_session_get_schema_meta((CassSession *) session->session->data),
                     free_schema);
}
